...
/* 0x37 */ JT_CASE(0x37) // FD_BPF_OP_DIV64_IMM
  register_file[instr.dst_reg] = instr.imm == 0 ? 0 : register_file[instr.dst_reg] / (int)instr.imm;
JT_CASE_END
...
/* 0x87 */ JT_CASE(0x87) // FD_BPF_OP_NEG64
  register_file[instr.dst_reg] = -(long)register_file[instr.dst_reg];
JT_CASE_END
/* 0x8d */ JT_CASE(0x8d) { // FD_BPF_OP_CALL_REG
  ...
  if( mem_region==FD_MEM_MAP_PROGRAM_REGION_START ) {
    ...
    register_file[10] += 0x2000;
    // FIXME: stack overflow fault.
    cond_fault = fd_vm_stack_push( &ctx->stack, pc, &register_file[6] );
    pc = (start_addr / 8UL)-1;
    pc -= ctx->instrs_offset;
  } else {
    fd_sbpf_syscalls_t * syscall_entry_reg = fd_sbpf_syscalls_query( ctx->syscall_map, register_file[instr.imm], NULL );
...
/* 0xdc */ JT_CASE(0xdc) // FD_BPF_OP_END_BE
  switch (instr.imm) {
    case 16:
      register_file[instr.dst_reg] = (ushort)fd_ushort_bswap((short)register_file[instr.dst_reg]);
      register_file[instr.dst_reg] &= 0xFFFF;
      break;
    case 32:
      register_file[instr.dst_reg] = (uint)fd_uint_bswap((int)register_file[instr.dst_reg]);
      register_file[instr.dst_reg] &= 0xFFFFFFFF;
      break;
    case 64:
      register_file[instr.dst_reg] = (ulong)fd_ulong_bswap((long)register_file[instr.dst_reg]);
      break;
  }
JT_CASE_END
