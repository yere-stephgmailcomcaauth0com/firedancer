...
void
propagate_head( fd_ghost_node_t * node, fd_ghost_node_t * old_head, fd_ghost_node_t * new_head ) {
  if( FD_UNLIKELY( FD_GHOST_NODE_EQ( old_head, new_head ) ) ) return;
  fd_ghost_node_t * ancestor = node;
  while( ancestor && FD_GHOST_NODE_EQ( ancestor->head, old_head ) ) {
    ancestor->head = new_head;
...
fd_ghost_replay_vote_upsert( fd_ghost_t *           ghost,
  ...
  if( FD_LIKELY( vote ) ) {
    ...
    if( FD_UNLIKELY( !node ) ) {
      ...
      /* Pubkey's prev vote is too old, as the voted slot hash has already been pruned from ghost.
       */
      ...
      while( ancestor->parent ) {
        fd_ghost_node_t * prev_heaviest = heaviest_child( ancestor->parent );
        ancestor->weight -= stake;
        fd_ghost_node_t * curr_heaviest = heaviest_child( ancestor->parent );
        propagate_head( ancestor, prev_heaviest->head, curr_heaviest->head );
        ancestor = ancestor->parent;
      }
    }
